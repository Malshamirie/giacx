...
class CreateProjectManagerConnectionsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('project_manager_connections', function (Blueprint $table) {
            ...
            $table->integer('from_manager_id')->unsigned(); // manager user id
            $table->integer('to_manager_id')->unsigned(); // manager user id
            ...
            // استخدام اسم أقصر للـ unique constraint
            $table->unique(['project_id', 'from_manager_id', 'to_manager_id'], 'pmc_unique_connection');
        });
    }
